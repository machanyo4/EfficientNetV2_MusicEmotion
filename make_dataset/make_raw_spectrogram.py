...
if not (os.path.exists(dataset_path + "/spec/Q1/raw")):
    os.mkdir(dataset_path + "/spec/Q1/raw")
if not (os.path.exists(dataset_path + "/spec/Q2/raw")):
    os.mkdir(dataset_path + "/spec/Q2/raw")
if not (os.path.exists(dataset_path + "/spec/Q3/raw")):
    os.mkdir(dataset_path + "/spec/Q3/raw")
if not (os.path.exists(dataset_path + "/spec/Q4/raw")):
    os.mkdir(dataset_path + "/spec/Q4/raw")
...
for file in Q1_filelist:
    ...
    raw_path = spec_dataset_path + '/Q1/raw/' + filename_no_extension + 'raw'
    for window_value in window_values:
        for overlap_rate in overlap_rates:
            ...
            if not os.path.exists(save_name):
                ...
                print("[LOG] " + filename + " -> /spec/Q1/raw/" +  filename_no_extension + param_data + ".png")
    print("--------------------")
...
for file in Q2_filelist:
    filename = os.path.basename(file)                       # filename を格納
    filename_no_extension = os.path.splitext(filename)[0]   # extention を排除して格納
    raw_path = spec_dataset_path + '/Q2/raw/' + filename_no_extension + 'raw'
    for window_value in window_values:
        for overlap_rate in overlap_rates:
            # パラメータ設定
            window = window_value
            ...
            if not os.path.exists(save_name):
                save_spectrogram(save_name, db, signal)
                print("[LOG] " + filename + " -> /spec/Q2/raw/" +  filename_no_extension + param_data + ".png")
    print("--------------------")
...
for file in Q3_filelist:
    filename = os.path.basename(file)                       # filename を格納
    filename_no_extension = os.path.splitext(filename)[0]   # extention を排除して格納
    raw_path = spec_dataset_path + '/Q3/raw/' + filename_no_extension + 'raw'
    for window_value in window_values:
        for overlap_rate in overlap_rates:
            # パラメータ設定
            window = window_value
            ...
            if not os.path.exists(save_name):
                save_spectrogram(save_name, db, signal)
                print("[LOG] " + filename + " -> /spec/Q3/raw/" +  filename_no_extension + param_data + ".png")
    print("--------------------")
...
for file in Q4_filelist:
    filename = os.path.basename(file)                       # filename を格納
    filename_no_extension = os.path.splitext(filename)[0]   # extention を排除して格納
    raw_path = spec_dataset_path + '/Q4/raw/' + filename_no_extension + 'raw'
    for window_value in window_values:
        for overlap_rate in overlap_rates:
            # パラメータ設定
            window = window_value
            ...
            if not os.path.exists(save_name):
                save_spectrogram(save_name, db, signal)
                print("[LOG] " + filename + " -> /spec/Q4/raw/" +  filename_no_extension + param_data + ".png")
    print("--------------------")
...
for file in unknown_filelist:
    filename = os.path.basename(file)                       # filename を格納
    filename_no_extension = os.path.splitext(filename)[0]   # extention を排除して格納
    raw_path = spec_dataset_path + '/unknown/' + filename_no_extension + 'raw'
    for window_value in window_values:
        for overlap_rate in overlap_rates:
            # パラメータ設定
            window = window_value
            ...
            if not os.path.exists(save_name):
                save_spectrogram(save_name, db, signal)
                print("[LOG] " + filename + " -> /spec/unknown/" +  filename_no_extension + param_data + ".png")
    print("--------------------")
...
# How many datas?    
print("[INFO] Datas in spec/Q1/raw: ", end='')
print(count_file(spec_dataset_path + "/Q1/raw"))
print("[INFO] Datas in spec/Q2/raw: ", end='')
print(count_file(spec_dataset_path + "/Q2/raw"))
print("[INFO] Datas in spec/Q3/raw: ", end='')
print(count_file(spec_dataset_path + "/Q3/raw"))
print("[INFO] Datas in spec/Q4/raw: ", end='')
print(count_file(spec_dataset_path + "/Q4/raw"))
